...
class PolarZeroSolver():
    ...
    def step_tangent(pos, grad, delta): # NOTE this is different
        # take a step perpendicular to the gradient (e.g. Euler-Lagrange)
        # In polar coords the Euclidean norm of a polar-vector v = [v_r, v_theta]
        # is sqrt(v_r^2 + (r * v_theta)^2).  Use that metric to normalise the
        # tangent direction so `delta` means Euclidean step-length.
        r = pos[..., 0]
        # avoid singular behaviour at r ~ 0
        r_safe = jnp.maximum(r, 1e-6)
        # build the perpendicular step in polar coordinate components
        step = jnp.array([1.0, -1.0]) * grad[::-1]
        # compute Euclidean norm under polar metric
        alpha = jnp.sqrt((step[..., 0])**2 + (r_safe * step[..., 1])**2)
        alpha = jnp.where(alpha == 0.0, 1.0, alpha)
    ...
    def stack_and_roll(init_pos, init_h, paths, paths_rev, roll_index):
        ...
        return {
            'path': PolarZeroSolver.vec_roll(
                PolarZeroSolver.stack(
                    paths_rev['path'][:, ::-1, :],
            ...
            'value': PolarZeroSolver.vec_roll(
                jnp.hstack([
                    paths_rev['value'][:, ::-1],
    ...
    def __init__(self, tol=1e-6, max_newton=5, forward_mode_differentiation=False):
        ...
        if self.forward_mode_differentiation:
            self.value_and_grad = PolarZeroSolver.value_and_grad_forward
            self.tangent_solve = PolarZeroSolver.tangent_solve_fwd
        else:
            self.value_and_grad = PolarZeroSolver.value_and_grad_rev
            self.tangent_solve = PolarZeroSolver.tangent_solve_rev
    ...
    def step_one_tp_inner(self, carry):
        ...
        pos = PolarZeroSolver.step_tangent(pos_in, grad, delta)
        pos, aux = self.newton(f, pos, 1)
        h = aux['value']
        grad = aux['grad']
        # if points are in polar coords (r,theta) convert to cartesian for distance
        pos_cart = PolarZeroSolver.polar_to_cartesian(pos)
        pos_in_cart = PolarZeroSolver.polar_to_cartesian(pos_in)
        pos_start_cart = PolarZeroSolver.polar_to_cartesian(pos_start)
        delta_travel = jnp.linalg.norm(pos_in_cart - pos_cart, axis=1)
        delta_start = jnp.linalg.norm(pos_start_cart - pos_cart, axis=1)
    ...
    def zero_contour_finder(self, f, init_guess, delta=0.1, N=1000, silent_fail=True):
        ...
        paths_fwd = PolarZeroSolver.trim_paths(jax.tree_util.tree_map(PolarZeroSolver.swap, paths_fwd), cut_index_fwd)
        end_points = PolarZeroSolver.vec_slice(paths_fwd['path'], cut_index_fwd, 0).squeeze()
        ...
        paths_rev = PolarZeroSolver.trim_paths(jax.tree_util.tree_map(PolarZeroSolver.swap, paths_rev), cut_index_rev)

        paths_combined = PolarZeroSolver.stack_and_roll(init_pos, init_h, paths_fwd, paths_rev, cut_index_rev)
...
def split_curves(a, threshold):
    ...
    '''
    # distance to next point -- handle polar (r,theta) by converting to cartesian first
    a_cart = PolarZeroSolver.polar_to_cartesian(a)
    d = jnp.sum(jnp.diff(a_cart, axis=0)**2, axis=1)
    jump = d > threshold
    ...
    for idx in range(cut_points.shape[0] - 1):
        ...
        cut_cart = PolarZeroSolver.polar_to_cartesian(cut)
        if jnp.sum((cut_cart[0] - cut_cart[-1])**2) < threshold:
