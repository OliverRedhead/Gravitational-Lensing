...
class Deflector:
    ...
    @staticmethod
    def euclid_resolution(arr, *, fov=2.0, res=0.1):
        """
        Reduces an array to the Euclid space telescope resolution.
        """
        ny, nx, *_ = arr.shape
        if nx != ny:
            raise ValueError(f"input array must be square. Not {nx}x{ny}")
    ...
    @staticmethod
    def set_resolution(arr, *, fov=2.0, telescope='hubble'):
        """
    ...
    @staticmethod
    def generate_noise(arr, t=600.0, gain=1, dark_level="low", signal_to_noise=500):
        """
        Apply a realistic HST CCD noise model to a normalized image.
